...
//   Algo: Here we use binary search to find the best height to cut , here we take l=0 and r=1e6(which is max size given in constraint)
// now use binary search search and check for mid if we get desired B or not, and based on the value of cur by mid
// decide to go left or right and reduce our search space.
